...
void searchPatientByPhoneNumber(const struct Patient patient[], int max)
{
    ...
    for (i = 0; i < max;i++)
    {
        if (strcmp(number, patient[i].phone.number) == 0)
        {
            index = i;
            cnt++;
        }
...
int nextPatientNumber(const struct Patient patient[], int max)
{
    ...
    for (i = 0;i < max;i++)
    {
        if (patient[i].patientNumber > nextPatient)
        {
            nextPatient = patient[i].patientNumber;
        }
...
void inputPhoneData(struct Phone* phone)
{
    ...
    if (selection == 1)
    {
        ...
        printf("Contact: %s\n", phone->description);
        printf("Number : ");
        inputCString(num, 1, PHONE_LEN);
        strcpy(phone->number, num);
        putchar('\n');
    }
    else if (selection == 2)
    {
